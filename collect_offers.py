...
def get_offers_items( browser, search_text, shops_ids=[] ): # int protection ?
    ...
    for shop_i in shops_ids:
        ...
        for sp in search_prop :
            ...
            for page_number in get_pages_list(    browser= browser.get(  f"{ shop_url }/{ sp }/{ formated_search }" ), pmi_item= pmi_item  ):
        
                browser.get(  f"{ shop_url }/{ sp }/{ formated_search }/{ page_ident }/{ page_number }"  )
                ...
                for offer_item in browser.find_elements( By.XPATH, offer_xpath ):
                    ...
                    offers[ shop_i ][ item_id ] = collect_attrs( offer_item, attrs_xpath )
...
def main():
    ...
    print( get_offers_items( browser, "Air jordan", shops_ids=[0] ) )
